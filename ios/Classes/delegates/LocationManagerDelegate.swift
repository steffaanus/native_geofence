...
/// Consolidated information about a background task
private struct BackgroundTaskInfo {
    let taskId: UIBackgroundTaskIdentifier
    let startTime: Date
    let eventParams: GeofenceCallbackParams
...
class LocationManagerDelegate: NSObject, CLLocationManagerDelegate {
    ...
    // Background task management - consolidated into single dictionary
    private var backgroundTasks: [UUID: BackgroundTaskInfo] = [:]
    ...
    private let maxConcurrentBackgroundTasks = 10  // iOS limit is usually ~10-20
    ...
    deinit {
        ...
        let tasksToClean = backgroundTasks
        backgroundTasks.removeAll()
        backgroundTasksLock.unlock()
        
        for (key, info) in tasksToClean {
            if info.taskId != .invalid {
                log.warning("Cleaning up leaked background task \(key) during deinit")
                persistFailedEvent(info.eventParams)
                UIApplication.shared.endBackgroundTask(info.taskId)
            }
        }
    }
    ...
    private func handleRegionEvent(region: CLRegion, event: GeofenceEvent) {
        ...
        // 2. CLEANUP STALE TASKS FIRST (PRIMARY STRATEGY)
        ...
        // 4. Create event params and store consolidated task info
        ...
        backgroundTasksLock.lock()
        backgroundTasks[taskKey] = taskInfo
        backgroundTasksLock.unlock()
    ...
    private func endBackgroundTask(taskKey: UUID) {
        ...
        guard let info = backgroundTasks.removeValue(forKey: taskKey) else {
        ...
        if info.taskId != .invalid {
            UIApplication.shared.endBackgroundTask(info.taskId)
            log.debug("Background task \(taskKey) ended successfully for geofence \(info.geofenceId)")
        }
    }
    
    /// Cleanup stale background tasks that have been running too long
    /// Called before every new geofence event (PRIMARY STRATEGY)
    ...
    /// Must be called while holding backgroundTasksLock
    private func cleanupAndPersistStaleTaskInternal(taskKey: UUID) {
        guard let info = backgroundTasks.removeValue(forKey: taskKey) else {
            return
        }
        
        let age = Date().timeIntervalSince(info.startTime)
        log.warning("Cleaning up stale background task \(taskKey) (age: \(String(format: "%.1f", age))s) for geofence \(info.geofenceId)")
        
        // Persist the event for retry
        persistFailedEvent(info.eventParams)
    ...
    /// Public wrapper for cleanup that acquires the lock
    private func cleanupAndPersistStaleTask(taskKey: UUID) {
        backgroundTasksLock.lock()
        defer { backgroundTasksLock.unlock() }
        cleanupAndPersistStaleTaskInternal(taskKey: taskKey)
    }
